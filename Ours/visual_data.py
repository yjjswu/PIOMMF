...
from ImitateData import load_ft
...
def load_feature_construct_H(data_dir,
    ...
    if use_mvcnn_feature:
        fts = hgut_gpu.feature_concat(fts, mvcnn_ft)
    if use_gvcnn_feature:
        fts = hgut_gpu.feature_concat(fts, gvcnn_ft)
    if fts is None:
    ...
    if use_mvcnn_feature_for_structure:
        tmp = hgut_gpu.construct_H_with_KNN(mvcnn_ft, K_neigs=K_neigs,
    ...
    if use_gvcnn_feature_for_structure:
        tmp = hgut_gpu.construct_H_with_KNN(gvcnn_ft, K_neigs=K_neigs,
                                        split_diff_scale=split_diff_scale,
                                        is_probH=is_probH, m_prob=m_prob)
        H = hgut_gpu.hyperedge_concat(H, tmp)
    if H is None:
