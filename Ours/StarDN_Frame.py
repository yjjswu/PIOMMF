...
class StarNet(nn.Module):
    def __init__(self, base_dim=32, depths=[3, 3, 12, 5], mlp_ratio=4, drop_path_rate=0.0, num_classes=1000, **kwargs):
        ...
        self.in_channel = 32
        # stem layer
        self.stem = nn.Sequential(ConvBN(64, self.in_channel, kernel_size=3, stride=1, padding=1), nn.ReLU6())
        ...
        # self.backbone=ghostnetv3(num_classes=num_classes,width=1.0)
        # self.backbone=MedViT_small(num_classes=num_classes)
        # self.backbone=torchvision.models.resnet50(num_classes=num_classes)
        # self.backbone.conv1=nn.Conv2d(32, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        ...
        # self.Conv_224To128=nn.Conv2d(in_channels=64,out_channels=64,kernel_size=5,stride=3,padding=2)
        self.relu=nn.ReLU()
        # self.HyperG = HGNN(50 * 50, 8, 512, 0.4)
    ...
    def forward(self, x,x_f):
        # G = self.Get_HyperGraph_2(x)
        #cewei_wowei,cewei_liwei,wowei_liwei=self.Get_HyperGraph_2(x)
        # List_W = self.Get_HyperGraph_2(x)
        # plt.figure(figsize=(10,10))
        # plt.imshow(List_W[0][0,0,:,:].cpu().detach().numpy())
        # plt.savefig("./cewei_wowei.png")
        # plt.figure(figsize=(10,10))
        # plt.imshow(List_W[1][0,0,:,:].cpu().detach().numpy())
        # plt.savefig("./cewei_liwei.png")
        # plt.figure(figsize=(10,10))
        # plt.imshow(List_W[2][0,0,:,:].cpu().detach().numpy())
        # plt.savefig("./wowei_liwei.png")
        # print(List_W[0].shape)
        # List_X=[x[:,0:1,:,:]*List_W[0]+x[:,0:1]*0.5,
        # x[:,1:2]*List_W[2]+x[:,1:2]*0.5,
        # x[:,2:3]*List_W[1]+x[:,2:3]*0.5]
        ...
        x_f=x_f+self.selfattn1(x_f)
        x_gen=self.gen(x_f)
        # GetImage.CheckImage(x[0])
        x=x+x_gen

        # print(x_gen.shape)
        # print(torch.flatten(self.avgpool(self.norm2(x)),1).shape,x_f.shape)
        # print(torch.flatten(self.avgpool(self.norm2(x)),1).shape,x.shape)
        # x = x
        # print(wieght.shape)
        # print(wieght.shape)
        # x=self.Conv_448To224(x)
        # x=self.relu(x)
        # print(x.shape)
        # x=self.Conv_224To128(x)
        # x=self.relu(x)
        # print(x.shape)
        # x=self.Get_HyperGraph(x)
        # print(x.shape)
        # plt.figure(figsize=(10,10))
        # plt.imshow(np.mean(x_gen[0].cpu().detach().numpy(),axis=0))
        ...
        # print("Gsize", G.shape, G.device)
        # G = G.reshape(G.shape[0], G.shape[1])
        # x = torch.flatten(self.norm(x), 1)
        x = torch.flatten(self.avgpool(x), 1)

        # print("xxxxx", x.shape)

        x_f2=self.relu(self.autodecoder(x_f))
        # x_temp = x_f2
        x_f2 = 0.5*self.selfattn2(x_f2)+0.5*x_f2+x
...
def Test_Model():
    model = starnet_s100(num_classes=2)
    Tensor1 = torch.rand(8, 3, 448, 448)
    A = model(Tensor1)
    print(A.shape)
