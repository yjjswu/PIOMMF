...
def train_model(model, dataloader, dataloaderx, num_classes, num_epochs=400):
    ...
    optimizer = optim.AdamW(model.parameters(), lr=0.001, weight_decay=0.12)
    scher1 = optim.lr_scheduler.StepLR(optimizer, step_size=24, gamma=0.4)
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, targets, features) in enumerate(dataloader):
            ...
            if (i % 5 == 0):
                ...
                with open('./run/MLPMaixer.txt', 'a') as file_loss:
                    file_loss.write(' ' + str(loss1.item()))
        scher1.step()

        torch.save(model, "./weights/MLPMaixer.pt")
...
# model=torch.load("./weights/OnlyTMIF.pt",weights_only=False).cuda()
train_model(model, dataloader, DataLoader2, num_classes, num_epochs=600)